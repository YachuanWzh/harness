...
flowchart LR
    ...
    PLUGIN -->|提供技能| SKILLS["/superharness:go<br/>/superharness:light<br/>/superharness:brainstorm<br/>+ 5 个核心技能"]
```
...
## 5. 会话启动流程（Claude Code 与 flavor-code）

Claude Code 由 `session-start.ps1|.sh` 输出 Hook JSON；flavor-code 由原生插件
`plugin/index.js` 返回同样的 `additionalContext`。flavor-code 1.2.20+ 会把该上下文持久写入
当前会话，并提供只读 `Skill` 工具，因此 `go` 在运行中引用的必需子技能会加载完整正文，而不是
只依赖技能名称或摘要。

```mermaid
flowchart TD
    OPEN([宿主启动会话]) --> LOAD["Claude: 本地 marketplace<br/>Flavor: .flavor/plugins/superharness"]
...
**跨宿主边界**：核心技能正文共享；插件包装、参数展开、Skill 调用、Hook 载荷和状态根路径由
宿主适配层负责。Flavor 低于 1.2.20 时只保证单个 Skill 的发现与显式调用，不宣称完整 `go` 等价。
...
flowchart LR
    ...
    P5 -->|有严重问题| P3
    P5 -->|通过| P6["⑥ 收尾<br/>finishing-a-development-branch<br/>合并分支 + 清理 worktree"]
    P6 --> DONE([完成])
```
...
### 6.2 收尾（finishing-a-development-branch）

Phase 5 审查通过后，`finishing-a-development-branch` 关闭隔离工作区：把验证过、
审查过的分支合并回主干（优先快进；主干已前进则正规合并并解决冲突），
`git worktree remove` 移除 worktree、`git branch -d` 删除已合并分支。合并后
**在主干再跑一次完整测试并贴出真实输出**——合并会改变代码树，不能只信任 worktree
里的结果。非 git 或原地工作则直接跳过。**不自动 push**——推送是人的决定；未合并的
分支绝不 `-D` 强删（`git branch -d` 拒绝即证明未合并，先查证）。

---

## 6.5 `light` 技能：轻量档（保留核心纪律，砍掉重装备）

`go` 六阶段对"改个文案、调个配置、修个小 bug"太重。`/superharness:light <任务目标>`
是配套的轻量档：**TDD（带明确豁免）+ 真实输出验证 + 根因调试** 三条核心纪律保留，
其余全部砍掉：

| go 的装备 | light 的处理 |
|-----------|--------------|
| worktree/分支隔离 | 默认原地工作；仅大改动建一次性分支（不建 worktree） |
| 计划文件 `plans/` | 不写，TodoWrite 管理任务清单 |
| 子代理实现/审查 | 主代理内联完成，收尾走自查清单 |
| Ralph 跟踪四文件 | 完全不启动——不写 `.current-task`，Stop 钩子自动 no-op，零记账 |

**设计要点**：
- **TDD 豁免是显式的**：纯配置、纯文案/文档、生成代码/脚手架、一次性原型可跳过
  RED-GREEN，但 Phase 3 验证不可省——"轻"不等于"不验证"。
- **零运行时状态**：light 不写 `.current-task`/`task.json`/`trace.jsonl`，中断重跑
  成本低，因此不需要 Ralph 的续跑能力——这是"轻"的主要来源，也让 Stop 钩子天然 no-op。
- **与 go 的互斥约定**：若已有活跃 go 任务（`.current-task` 存在），light 会先提醒，
  不接管、不结束它。
- **自动升级路径**：任务变大（触及多个子系统、需要正式审查）时，light 主动建议切换回
  `/superharness:go`。
...
持久化进**下文 7.5 的 Ralph 四文件状态**（`.claude/superharness/ralph/`），并在失败时**于同一次运行内
...
| 谁 | 触发/时机 | 干什么 |
|----|-----------|--------|
| `UserPromptSubmit` hook | 每次用户提交 prompt | 暂存 `{ts, query}` 到 `.claude/superharness/ralph/.pending-prompt.json` |
...
落在**目标项目**里（与 `.claude/superharness/plans/` 对齐，整目录已由安装器写入 `.gitignore`）：
...
`scripts/ralph-lib.ps1`（dot-source 即用），在 `<项目>/.claude/superharness/ralph/` 下维护四个文件：
...
对缺失/损坏文件返回 `$null`；运行时四个文件位于 `.claude/superharness/ralph/`（已加入 `.gitignore`）。
...
flowchart LR
    ...
    subgraph FILES["会话目录 .claude/superharness/brainstorm/<session>"]
...
sequenceDiagram
    ...
    C->>S: 下一轮读取 events
    C->>U: 结合终端文字理解意图，继续推进
    Note over C,B: 结束后生成设计文档到 .claude/superharness/specs/
```
...
- **会话产物**：落在 `.claude/superharness/brainstorm/`（安装器已写入 `.gitignore`），不污染仓库。
...
flowchart TD
    ...
    F -->|每轮被 hook 记录| T[".claude/superharness/ralph/trace.jsonl<br/>(+ task.json / .current-task / .ralph-state.json)"]
    F -->|测试失败| RS["同一次运行内自动重试<br/>复现→定位→改码→验证 (上限 5)"]
    RS --> F
    F --> H([合并分支 + 清理 worktree，提交经验证、经审查的成果])
    G --> I([生成设计文档到 .claude/superharness/specs])
    I -.可转.-> F
```
